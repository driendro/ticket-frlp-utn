...
class Usuario_model extends CI_Model
{
	...
	{
		return $this->db->select('*')->where('legajo', $legajo)->get('usuario')->row();
	}
	...
	{
		$saldoActual = $this->db->select('saldo')->where('id_usuario', $iduser)->get('usuario')->row('saldo');
		$saldoNuevo = $saldoActual + $saldo;

		$this->db->set('saldo', $saldoNuevo)->where('id_usuario', $iduser)->update('usuario');
	}
