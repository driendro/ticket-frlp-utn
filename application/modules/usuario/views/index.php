    				...
    				<form method="post" action="<?= base_url('datos'); ?>" id="formCompraId">
    					...
    					<?php foreach ($dias as $key => $dia) : ?>
    						...
    						<div class="my-1 form-check form-check-inline">
    							<fieldset <?= ($usuario->saldo < 180) || (in_array(date('Y-m-d', $proximo), array_column($comprados, 'dia_comprado'))) || (in_array(date('Y-m-d', $proximo), array_column($feriados, 'fecha'))) ? 'disabled' : ''; ?>>
